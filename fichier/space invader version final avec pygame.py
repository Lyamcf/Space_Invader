...
# installer pygame executeur de command :
...
nb_av_enmt=20#variable d'optimisation qui donne le nombre d'avion enmie qui peux avoir en meme dans sur l'ecran
...
stage_song = pygame.mixer.Sound("stage suivant.wav")
...
pygame.mixer.Sound.set_volume(stage_song,volume_effect_sonore)
...
def tire_laser(evt):
    global n,Tt,llance_balle,stage
    ...
    if evt.keysym==Tt and llance_balle == "true":
        ...
        timmer = float(time())
        timer_balle(timmer)
        #permet de lancer une ou plusieur balle selon le stage ainsi que changer l'image
        if stage<3 :
            n=canvas1.create_image(xAJ,yAJ-15,image=laser) # creer la munition avec un nom different "n"
            ...
            n=canvas1.create_image(xAJ,yAJ-15,image=laser3) # creer la munition avec un nom different "n"
            mouvement_laser(xAJ,yAJ,n)
...
def jouer():
    ...
    def stage_fct(songstage):
        ...
        if score < 10:
            ...
            stage = 6
            temps_enemie = randint(200,1000)
            tps_tir_balle=0.1
            vitesse_avion_enemie = 2.5
        else :
            temps_enemie = randint(250,500)
            tps_tir_balle=0.1
            vitesse_avion_enemie = 3
        if songstage != stage :
            #pygame.mixer.Sound.play(stage_song)
            songstage = stage
        canvas1.after(750,stage_fct,songstage)
...
def perdu():
    ...
    Lab_meilleur_score.place(x=350,y=20)
def musique ():
    global musique_activate,effect_activate,volume_effect_sonore,volume_musique,mlancer
    '''
    actualisation de la musique et des effect sonore ainsi que les volumes
    '''
    if (musique_activate == "true" and mlancer == "true"):
        mlancer = "false"
    ...
    if effect_activate == "true":
        ...
        pygame.mixer.Sound.set_volume(piece_sound,0)
        pygame.mixer.Sound.set_volume(letgo_sound,0)
        pygame.mixer.Sound.set_volume(gameover_sound,0)
        pygame.mixer.Sound.set_volume(stage_song,0)
        pygame.mixer.Sound.set_volume(tire_laser_sound,0)
    if jeu == "false":
        Mafenetre.after(1500,musique)
...
def stream(label):
    ...
    for image in video.iter_data():
        frame_image = ImageTk.PhotoImage(Image.fromarray(image),width = 850,height = 800)
...
QUITTERS=Button(canevas,text="QUITTER",bg="purple",fg="black",font=f,command= quitter)
...
Lab_meilleur_score.place(x=350,y=20)
QUITTERS.place(x=700,y=650)

musique()

if __name__ == "__main__":
    my_label = tk.Label(Mafenetre,width = 850,height = 800)
    my_label.pack()
...
sMafenetre.bind_all("<Key>",touchechoix)
Mafenetre.mainloop() 